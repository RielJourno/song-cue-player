...
import { getSong, getPersonalCues, savePersonalCues } from '../db';
import type { Song, Cue, DisplayCue, SharedSongJSON } from '../types';
...
export default function SongScreen() {
  ...
  // Personal cues for shared songs — loaded from IndexedDB, never in song.cues
  const [personalCues, setPersonalCues] = useState<Cue[]>([]);
  ...
  const [editingCue, setEditingCue] = useState<DisplayCue | null>(null);
  ...
  // In author mode, shared cues become editable. Otherwise only personal/local cues are editable.
  const canEditSharedCues = isShared && authorMode;

  // Merged display list: shared cues (blue) + personal cues (amber), sorted by time
  const displayCues: DisplayCue[] = [
    ...(song?.cues ?? []).map((c) => ({ ...c, isShared: true })),
    ...personalCues.map((c) => ({ ...c, isShared: false })),
  ...
  useEffect(() => {
    ...
    if (sharedSong && id === sharedSong.id) {
      ...
      // Load personal cues for this shared song
      getPersonalCues(sharedSong.id).then((rec) => {
        setPersonalCues(rec?.cues ?? []);
      });
      return;
    }
  ...
  // Keep in sync when store's sharedSong changes (author mode edits)
  useEffect(() => {
  ...
  // iOS Safari requires explicit load() when src attribute changes
  useEffect(() => {
  ...
  function togglePlay() {
    ...
    if (isPlaying) {
      ...
      const p = audio.play();
      if (p !== undefined) {
        p.then(() => { setIsPlaying(true); acquireWakeLock(); })
          .catch((e) => { showToast('Playback failed. Try tapping again.'); console.error(e); });
      }
    }
  }
  ...
  function jumpToCue(cue: Cue) {
    ...
    if (!isPlaying) {
      const p = audio.play();
      if (p !== undefined) {
        p.then(() => { setIsPlaying(true); acquireWakeLock(); }).catch(() => {});
      }
    }
  }
  ...
  // Persist shared cues to store (author mode) or local song to IndexedDB
  async function persistSharedCues(updatedSong: Song) {
    setSong(updatedSong);
    updateSharedSongCues(updatedSong);
  }

  async function persistLocalSong(updated: Song) {
    setSong(updated);
    await updateSong(updated);
  }

  // Persist personal cues for the current shared song
  async function persistPersonalCues(cues: Cue[]) {
    if (!song) return;
    setPersonalCues(cues);
    await savePersonalCues({ songId: song.id, cues, updatedAt: Date.now() });
  }
  ...
  async function submitTitle() {
    ...
    if (trimmed && trimmed !== song.title) {
      const updated = { ...song, title: trimmed, updatedAt: Date.now() };
      if (isShared) await persistSharedCues(updated);
      else await persistLocalSong(updated);
    } else {
  ...
  async function addCueAtCurrentTime() {
    ...
    if (isShared && canEditSharedCues) {
      // Author mode: new cue goes to shared list
      const updated = { ...song, cues: [...song.cues, cue].sort((a, b) => a.timeSeconds - b.timeSeconds), updatedAt: Date.now() };
      await persistSharedCues(updated);
    } else if (isShared) {
      // Non-author: new cue is personal
      const sorted = [...personalCues, cue].sort((a, b) => a.timeSeconds - b.timeSeconds);
      await persistPersonalCues(sorted);
    } else {
      // Local song
      const updated = { ...song, cues: [...song.cues, cue].sort((a, b) => a.timeSeconds - b.timeSeconds), updatedAt: Date.now() };
      await persistLocalSong(updated);
    }
    setNewInlineCue(cue);
  }
  ...
  async function saveInlineCueLabel(cue: Cue, label: string) {
    ...
    if (isShared && canEditSharedCues && song.cues.some((c) => c.id === cue.id)) {
      // Author editing a shared cue label
      const updated = { ...song, cues: song.cues.map((c) => c.id === cue.id ? { ...c, label: trimmed } : c), updatedAt: Date.now() };
      await persistSharedCues(updated);
    } else if (isShared) {
      // Personal cue label
      const updated = personalCues.map((c) => c.id === cue.id ? { ...c, label: trimmed } : c);
      await persistPersonalCues(updated);
    } else {
      const updated = { ...song, cues: song.cues.map((c) => c.id === cue.id ? { ...c, label: trimmed } : c), updatedAt: Date.now() };
      await persistLocalSong(updated);
    }
  ...
  async function submitImport() {
    ...
    if (isShared && canEditSharedCues) {
      ...
      await persistSharedCues({ ...song, cues: merged, updatedAt: Date.now() });
    } else if (isShared) {
      const merged = [...personalCues, ...newCues].sort((a, b) => a.timeSeconds - b.timeSeconds);
      await persistPersonalCues(merged);
    } else {
      const merged = [...song.cues, ...newCues].sort((a, b) => a.timeSeconds - b.timeSeconds);
      await persistLocalSong({ ...song, cues: merged, updatedAt: Date.now() });
    }
  ...
  function openCueEdit(dc: DisplayCue) {
    if (dc.isShared && !canEditSharedCues) {
      showToast('Cue משותף — אפשר רק לקפוץ אליו');
      return;
    }
    setEditingCue(dc);
    setEditCueLabel(dc.label);
    setEditCueTime(formatTime(dc.timeSeconds));
  }
  ...
  async function submitCueEdit() {
    ...
    if (editingCue.isShared) {
      // Author mode editing a shared cue
      const updated = {
        ...song,
      ...
      // Editing a personal cue on a shared song
      const updated = personalCues.map((c) => c.id === editingCue.id ? { ...c, label, timeSeconds: t } : c)
        .sort((a, b) => a.timeSeconds - b.timeSeconds);
      await persistPersonalCues(updated);
    } else {
      // Editing a cue on a local song
      const updated = {
        ...song,
        cues: song.cues.map((c) => c.id === editingCue.id ? { ...c, label, timeSeconds: t } : c)
          .sort((a, b) => a.timeSeconds - b.timeSeconds),
  ...
  async function deleteCue(dc: DisplayCue) {
    if (!song) return;
    if (dc.isShared && !canEditSharedCues) {
      showToast('Cue משותף — אפשר רק לקפוץ אליו');
      return;
    ...
    if (dc.isShared) {
      const updated = { ...song, cues: song.cues.filter((c) => c.id !== dc.id), updatedAt: Date.now() };
      await persistSharedCues(updated);
    } else if (isShared) {
      await persistPersonalCues(personalCues.filter((c) => c.id !== dc.id));
    } else {
      const updated = { ...song, cues: song.cues.filter((c) => c.id !== dc.id), updatedAt: Date.now() };
      await persistLocalSong(updated);
    }
  ...
  async function handleExportCues() {
    if (!song) return;
    // Export all visible cues (shared + personal) as text
    const text = displayCues.map((c) => `${formatTime(c.timeSeconds)} ${c.label}`).join('\n');
    if (!text) { showToast('No cues to export.'); return; }
    try {
  ...
  // Author mode: export ONLY shared cues as cues.json
  async function handleExportCuesJson() {
    ...
    const json: SharedSongJSON = {
      ...
      cues: song.cues, // shared cues only — never includes personalCues
      updatedAt: Date.now(),
    };
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0a] text-white">
      {/* <audio> always in DOM — src driven by state, never conditionally rendered */}
      <audio
        ref={audioRef}
      ...
      <header className="flex items-center px-4 pt-4 pb-3 gap-3 border-b border-white/10">
        ...
        {editingTitle && (isShared ? canEditSharedCues : true) ? (
          <input
            autoFocus
          ...
          <button
            ...
            onClick={() => (isShared ? canEditSharedCues : true) && setEditingTitle(true)}
          >
            {song?.title ?? '…'}
          </button>
        )}
        ...
        <button onClick={() => setShowOptions(true)} className="w-10 h-10 flex items-center justify-center text-white/40 active:text-white" aria-label="Song options">
      ...
      {notFound ? (
        ...
        <>
          ...
          <div className="px-5 pt-5 pb-4 flex flex-col gap-4">
            ...
            {/* Progress bar with two-color tick marks */}
            <div className="relative">
              <input
                ...
                style={{ background: `linear-gradient(to right, #f59e0b ${progressPct}%, rgba(255,255,255,0.2) ${progressPct}%)` }}
              />
              {isShared ? (
                <>
                  {/* Shared cues — blue ticks */}
                  {song.cues.map((cue) => (
                    <div key={`s-${cue.id}`}
                      className="absolute top-1/2 -translate-y-1/2 w-1 h-3 rounded-full pointer-events-none"
                      style={{ left: `calc(${duration > 0 ? (cue.timeSeconds / duration) * 100 : 0}% - 2px)`, backgroundColor: '#60a5fa80' }}
                    />
                  ))}
                  {/* Personal cues — amber ticks */}
                  {personalCues.map((cue) => (
                    <div key={`p-${cue.id}`}
                      className="absolute top-1/2 -translate-y-1/2 w-1 h-3 rounded-full pointer-events-none"
                      style={{ left: `calc(${duration > 0 ? (cue.timeSeconds / duration) * 100 : 0}% - 2px)`, backgroundColor: '#f59e0b80' }}
                    />
                  ))}
                </>
              ) : (
                /* Local song — all amber */
                song.cues.map((cue) => (
                  <div key={cue.id}
          ...
          <div className="flex-1 overflow-y-auto px-4">
            ...
            <ul className="flex flex-col gap-2 pb-4">
              {displayCues.map((dc) => (
                <li key={dc.id}>
                  {newInlineCue?.id === dc.id ? (
                    <InlineCueEditor
                      initialLabel={dc.label}
                      onSave={(label) => saveInlineCueLabel(dc, label)}
                      onCancel={() => saveInlineCueLabel(dc, dc.label)}
                    />
                  ) : (
                    <CueButton
                      cue={dc}
                      canEdit={dc.isShared ? canEditSharedCues : true}
                      onTap={() => jumpToCue(dc)}
                      onEdit={() => openCueEdit(dc)}
                      onDelete={() => deleteCue(dc)}
                      onBlockedTap={() => showToast('Cue משותף — אפשר רק לקפוץ אליו')}
                    />
                  )}
                </li>
              ))}
          ...
          {/* Bottom bar — always shown; writes to personal on shared songs */}
          <div
            className="border-t border-white/10 bg-[#0a0a0a] px-4 pt-3 flex gap-2"
            style={{ paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom))' }}
          >
            <button onClick={addCueAtCurrentTime}
            ...
            <button onClick={() => { setManualLabel(''); setManualTime(''); setShowManualDialog(true); }}
              className="min-h-[52px] px-3 bg-white/10 text-white rounded-xl active:bg-white/20 text-sm">
              Manual
            </button>
            <button onClick={() => { setImportText(''); setShowImportDialog(true); }}
              className="min-h-[52px] px-3 bg-white/10 text-white rounded-xl active:bg-white/20 text-sm">
              Import
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-5">
          <div className="w-full max-w-sm bg-[#1a1a1a] rounded-2xl p-5 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input autoFocus className="bg-white/10 text-white rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-amber-500"
                value={editCueLabel} onChange={(e) => setEditCueLabel(e.target.value)} placeholder="Cue label" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-white/50 text-sm">Time (mm:ss)</label>
              <input className="bg-white/10 text-white rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-amber-500"
                value={editCueTime} onChange={(e) => setEditCueTime(e.target.value)} placeholder="1:23" />
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-5">
          <div className="w-full max-w-sm bg-[#1a1a1a] rounded-2xl p-5 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-white/50 text-sm">Label</label>
              <input autoFocus className="bg-white/10 text-white rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-amber-500"
                value={manualLabel} onChange={(e) => setManualLabel(e.target.value)} placeholder="e.g. Parents dance" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-white/50 text-sm">Time (mm:ss)</label>
              <input className="bg-white/10 text-white rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-amber-500"
                value={manualTime} onChange={(e) => setManualTime(e.target.value)} placeholder="1:23"
      ...
      {showImportDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-5">
          <div className="w-full max-w-sm bg-[#1a1a1a] rounded-2xl p-5 flex flex-col gap-4">
            ...
            <textarea autoFocus rows={8} spellCheck={false}
      ...
      {showOptions && (
        <div className="fixed inset-0 z-50 flex items-end bg-black/60" onClick={() => setShowOptions(false)}>
          <div className="w-full bg-[#1a1a1a] rounded-t-2xl p-4 pb-8" onClick={(e) => e.stopPropagation()}>
            ...
            {isShared && authorMode && (
              <button className="w-full text-left py-4 px-4 text-amber-400 text-base rounded-xl active:bg-white/10" onClick={handleExportCuesJson}>
                Export cues.json
              </button>
            )}
            <button className="w-full text-left py-4 px-4 text-white text-base rounded-xl active:bg-white/10"
              onClick={() => { setShowOptions(false); handleExportCues(); }}>
              <span className="flex items-center gap-3"><span className="text-lg">↗</span>Export cues</span>
            </button>
            {!isShared && (
              <button className="w-full text-left py-4 px-4 text-red-400 text-base rounded-xl active:bg-white/10" onClick={handleDeleteSong}>
                Delete song
              </button>
            )}
            <button className="w-full text-left py-4 px-4 text-white/50 text-base rounded-xl active:bg-white/10" onClick={() => setShowOptions(false)}>
              Cancel
            </button>
          </div>
        </div>
...
function CueButton({
  cue, canEdit, onTap, onEdit, onDelete, onBlockedTap,
}: {
  cue: DisplayCue; canEdit: boolean;
  onTap: () => void; onEdit: () => void; onDelete: () => void; onBlockedTap: () => void;
}) {
  const [open, setOpen] = useState(false);
  const timeColor = cue.isShared ? '#60a5fa' : '#f59e0b';
    ...
    return (
      <div className="min-h-[64px] flex items-center bg-[#1a1a1a] rounded-xl overflow-hidden">
        <div className="flex-1 px-4 py-3">
          ...
          <div className="text-sm" style={{ color: timeColor }}>{formatTime(cue.timeSeconds)}</div>
        </div>
        <button onClick={() => { setOpen(false); onEdit(); }} className="h-full px-5 py-3 text-white/80 text-sm font-medium border-l border-white/10 active:bg-white/10">Edit</button>
        <button onClick={() => { setOpen(false); onDelete(); }} className="h-full px-5 py-3 text-red-400 text-sm font-medium border-l border-white/10 active:bg-white/10">Delete</button>
        <button onClick={() => setOpen(false)} className="h-full px-4 py-3 text-white/30 border-l border-white/10 active:bg-white/10" aria-label="Close">✕</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-[64px] flex items-center bg-[#1a1a1a] rounded-xl overflow-hidden">
      ...
      {/* "משותף" badge on shared cues */}
      {cue.isShared && (
        <span className="text-[11px] text-blue-400/60 px-3 flex-shrink-0 font-medium">משותף</span>
      )}
      {/* Edit button: show for editable cues, show blocked indicator for shared cues in read-only */}
      <button
        onClick={() => { canEdit ? setOpen(true) : onBlockedTap(); }}
        className="w-12 h-full flex items-center justify-center text-white/20 active:text-white border-l border-white/10 active:bg-white/10 self-stretch"
...
function InlineCueEditor({ initialLabel, onSave, onCancel }: { initialLabel: string; onSave: (l: string) => void; onCancel: () => void }) {
  ...
  return (
    <div className="min-h-[64px] flex items-center gap-2 px-4 py-3 bg-[#252525] rounded-xl">
      <input autoFocus className="flex-1 bg-transparent text-white outline-none placeholder-white/30"
        value={value} onChange={(e) => setValue(e.target.value)} placeholder="Cue label"
